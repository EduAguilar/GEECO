...
@IonicPage()
@Component({
  selector: 'page-modal-nuevo-ingreso',
  templateUrl: 'modal-nuevo-ingreso.html',
})
export class ModalNuevoIngresoPage {
  importe: any = '';
  tipo: string = ''; 
  categoria: string = '';
  nota: any = '';
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad ModalNuevoIngresoPage');
  }
  ...
  guardarIngreso(){
    ...
    this.db.cargarIngreso(ingreso).then((res)=>{
      this.cerrarModal();
